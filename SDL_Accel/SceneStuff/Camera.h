...
//    ray.direction = addVector(addVector(a, b), cc);
    ray = newRay(addVector(addVector(a, b), cc), c.position);
//    ray.direction = addVector(addVector(scaleVector((x - w * 0.5)/w*ar, camera_Right_Vec(c)), scaleVector((h * 0.5 - y)/h, camera_Up_Vec(c))), camera_Forward_Vec(c));
    return ray;
}
